...
function chassis(l, w, h, wheelR, hubR, frontWheelOffset, backWheelOffset, windAngle, windAngle2, servoCenterToRightDist, servoCenterToTopDist, servoCenterToLeftDist) {
  return [
    difference(
    union(
        difference(
...
// cut out space for electroinics
            translate([-l/2 + backWheelOffset, -w/2 + 3,wheelR], 
                rotate([45,0,0],
                    cube([l - (backWheelOffset + frontWheelOffset),Math.sqrt(Math.pow((w - 6),2)/2),-Math.sqrt(Math.pow((w - 6),2)/2)]))
                ),
//make space for bord
            translate([-l/2 + backWheelOffset, -w/2 + 5,wheelR],
                    cube([l - (backWheelOffset + frontWheelOffset),w - 10,5])
                )
            ),
// add space for servos
//servoCenterToRightDist, servoCenterToTopDist, servoCenterToLeftDist
        translate([-l/2 + backWheelOffset, -w/2 + 3,wheelR + 3], 
                cube([servoCenterToRightDist + servoCenterToLeftDist + 4,w-6,-servoCenterToTopDist])
            ),
// front axle
        translate([l/2 - frontWheelOffset - wheelR, -w/2, wheelR], 
            axel(w,hubR)
...
// bacl wheels
 ...
 // servo space
        translate([-l/2 + backWheelOffset + 2, -w/2 + 3,wheelR + 3], 
            cube([servoCenterToRightDist + servoCenterToLeftDist,w-6,-servoCenterToTopDist])
        ),
        translate([-l/2 + backWheelOffset, -w/2 + 5,wheelR + 3], 
            cube([servoCenterToRightDist + servoCenterToLeftDist + 4,w-10,-servoCenterToTopDist])
        )
        )
    ]
}
...
function main(params) {
  ...
  //return (translate([-l/2,-w/2,0],cube([l,w,h])));
    ...
    var carChasis = chassis(l, w, h, wheelR, hubR, frontWheelOffset, backWheelOffset, windAngle, windAngle2, servoCenterToRightDist, servoCenterToTopDist, servoCenterToLeftDist);
    return (carChasis );
    /*return (translate([-l/2 + backWheelOffset, -w/2 + 3,wheelR], 
                    cube([servoCenterToRightDist + servoCenterToLeftDist + 4,w-6,-servoCenterToTopDist])
                )
    )
    */
    /*return (translate([-l/2 + backWheelOffset, -w/2 + 3], 
                rotate([45,0,0],
                    cube([1,w,w]))
                )
           );*/
